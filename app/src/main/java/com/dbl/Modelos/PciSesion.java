package com.dbl.Modelos;

public class PciSesion {

    private static PciSesion mInstance;

    public static PciSesion getInstance() {
        if (mInstance == null) {
            mInstance = new PciSesion();
        }
        return mInstance;
    }

    public static void resetSesion() {
        mInstance = null;
    }

    private long id;
    private String ct;
    private String mt;
    private String direccion;
    private String medidor;
    private String medidorAnterior;
    private String medidorPosterior;
    private String barrio;
    private String lectura;
    private long anomalia;
    private String observacionAnalisis;
    private String municipio;
    private String codigo;
    private String anAnterior;
    private String lecturaAnterior;
    private long unicom;
    private long ruta;
    private long itin;
    private String latitud;
    private String longitud;
    private long orden;
    private String foto;
    private String fechaRealizado;
    private long lectorAsignadoId;
    private long lectorRealizaId;
    private long estado;
    private long lastInsert;

    private boolean observacionObligatoria;

    public long getId() {
        return id;
    }

    public void setId(long id) {
        this.id = id;
    }

    public String getCt() {
        return ct;
    }

    public void setCt(String ct) {
        this.ct = ct;
    }

    public String getMt() {
        return mt;
    }

    public void setMt(String mt) {
        this.mt = mt;
    }

    public String getDireccion() {
        return direccion;
    }

    public void setDireccion(String direccion) {
        this.direccion = direccion;
    }

    public String getMedidor() {
        return medidor;
    }

    public void setMedidor(String medidor) {
        this.medidor = medidor;
    }

    public String getMedidorAnterior() {
        return medidorAnterior;
    }

    public void setMedidorAnterior(String medidorAnterior) {
        this.medidorAnterior = medidorAnterior;
    }

    public String getMedidorPosterior() {
        return medidorPosterior;
    }

    public void setMedidorPosterior(String medidorPosterior) {
        this.medidorPosterior = medidorPosterior;
    }

    public String getBarrio() {
        return barrio;
    }

    public void setBarrio(String barrio) {
        this.barrio = barrio;
    }

    public String getLectura() {
        return lectura;
    }

    public void setLectura(String lectura) {
        this.lectura = lectura;
    }

    public long getAnomalia() {
        return anomalia;
    }

    public void setAnomalia(long anomalia) {
        this.anomalia = anomalia;
    }

    public String getObservacionAnalisis() {
        return observacionAnalisis;
    }

    public void setObservacionAnalisis(String observacionAnalisis) {
        this.observacionAnalisis = observacionAnalisis;
    }

    public String getMunicipio() {
        return municipio;
    }

    public void setMunicipio(String municipio) {
        this.municipio = municipio;
    }

    public String getCodigo() {
        return codigo;
    }

    public void setCodigo(String codigo) {
        this.codigo = codigo;
    }

    public String getAnAnterior() {
        return anAnterior;
    }

    public void setAnAnterior(String anAnterior) {
        this.anAnterior = anAnterior;
    }

    public String getLecturaAnterior() {
        return lecturaAnterior;
    }

    public void setLecturaAnterior(String lecturaAnterior) {
        this.lecturaAnterior = lecturaAnterior;
    }

    public long getUnicom() {
        return unicom;
    }

    public void setUnicom(long unicom) {
        this.unicom = unicom;
    }

    public long getRuta() {
        return ruta;
    }

    public void setRuta(long ruta) {
        this.ruta = ruta;
    }

    public long getItin() {
        return itin;
    }

    public void setItin(long itin) {
        this.itin = itin;
    }

    public String getLatitud() {
        return latitud;
    }

    public void setLatitud(String latitud) {
        this.latitud = latitud;
    }

    public String getLongitud() {
        return longitud;
    }

    public void setLongitud(String longitud) {
        this.longitud = longitud;
    }

    public long getOrden() {
        return orden;
    }

    public void setOrden(long orden) {
        this.orden = orden;
    }

    public String getFoto() {
        return foto;
    }

    public void setFoto(String foto) {
        this.foto = foto;
    }

    public String getFechaRealizado() {
        return fechaRealizado;
    }

    public void setFechaRealizado(String fechaRealizado) {
        this.fechaRealizado = fechaRealizado;
    }

    public long getLectorAsignadoId() {
        return lectorAsignadoId;
    }

    public void setLectorAsignadoId(long lectorAsignadoId) {
        this.lectorAsignadoId = lectorAsignadoId;
    }

    public long getLectorRealizaId() {
        return lectorRealizaId;
    }

    public void setLectorRealizaId(long lectorRealizaId) {
        this.lectorRealizaId = lectorRealizaId;
    }

    public long getEstado() {
        return estado;
    }

    public void setEstado(long estado) {
        this.estado = estado;
    }

    public long getLastInsert() {
        return lastInsert;
    }

    public void setLastInsert(long lastInsert) {
        this.lastInsert = lastInsert;
    }

    public boolean isObservacionObligatoria() {
        return observacionObligatoria;
    }

    public void setObservacionObligatoria(boolean observacionObligatoria) {
        this.observacionObligatoria = observacionObligatoria;
    }
}
